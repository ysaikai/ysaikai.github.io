...
* Efficient learning of site-specific management in precision agriculture
  ...
  - [Abstract]<br>
    While precision agriculture (PA) holds a promise of modern agriculture: increased profitability and environmental sustainability, farmers face new opportunities and challenges. They could achieve greater efficiency by optimizing site-specific management based on the unique information on each site obtained through various sensor technologies. At the same time, since modern PA equipment often requires substantial investment, without sufficient return generated by effective use of the technology, PA might merely increase financial risks and even lower profitability. To accelerate its adoption, it is crucial for farmers to perceive concrete benefits under their own unique environments. To this end, we propose a model designed for profit maximization in PA as well as a general yet practical solution algorithm. The algorithm is based on a class of numerical optimization techniques called Bayesian optimization, which is known for its sample efficiency and capability to handle complex objective functions. The results indicate that, in a wide range of simulated environments, our approach to site-specific management is indeed more profitable than the conventional uniform management.
